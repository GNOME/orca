...
class Utilities(script_utilities.Utilities):
    ...
    def isDocument(self, obj, excludeDocumentFrame=True):
        ...
        try:
            rv = Atspi.Accessible.get_role(obj) in roles
        except:
    ...
    def grabFocusWhenSettingCaret(self, obj):
        ...
        if role == Atspi.Role.IMAGE:
            isLink = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.LINK
    ...
    def nodeLevel(self, obj):
        ...
        rv = -1
        if not (self.inMenu(obj) or Atspi.Accessible.get_role(obj) == Atspi.Role.HEADING):
            attrs = self.objectAttributes(obj)
    ...
    def getPositionInSet(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) == Atspi.Role.TABLE_ROW:
            rowindex = attrs.get('rowindex')
            if rowindex is None and obj.childCount:
                roles = self._cellRoles()
                cell = AXObject.find_descendant(obj, lambda x: x and Atspi.Accessible.get_role(x) in roles)
    ...
    def isNonEntryTextWidget(self, obj):
        ...
        if role in roles:
            ...
            rv = Atspi.Accessible.get_role(obj.parent) != Atspi.Role.LIST
    ...
    def isNonInteractiveDescendantOfControl(self, obj):
        ...
        if role in roles and not state.contains(Atspi.StateType.FOCUSABLE):
            ...
            rv = AXObject.find_ancestor(obj, lambda x: x and Atspi.Accessible.get_role(x) in controls)
    ...
    def _getTextAtOffset(self, obj, offset, boundary):
        ...
        if boundary == Atspi.TextBoundaryType.SENTENCE_START \
            ...
            allText = text.getText(0, -1)
            if Atspi.Accessible.get_role(obj) in [Atspi.Role.LIST_ITEM, Atspi.Role.HEADING] \
    ...
    def getWordContentsAtOffset(self, obj, offset, useCache=True):
        ...
        firstObj, firstStart, firstEnd, firstString = objects[0]
        if firstStart == 0 and Atspi.Accessible.get_role(firstObj) == Atspi.Role.LIST_ITEM:
            objects = [objects[0]]
    ...
    def getLineContentsAtOffset(self, obj, offset, layoutMode=None, useCache=True):
        ...
        offset = max(0, offset)
        if Atspi.Accessible.get_role(obj) == Atspi.Role.TOOL_BAR and not self._treatObjectAsWhole(obj):
            child = self.getChildAtOffset(obj, offset)
            if child:
                obj = child
                offset = 0
        ...
        def _include(x):
            ...
            if obj != xObj:
                ...
                if self.isLink(obj) and self.isLink(xObj):
                    ...
                    return False
                elif Atspi.Accessible.get_role(obj) in [Atspi.Role.TREE, Atspi.Role.TREE_ITEM] \
                     and Atspi.Accessible.get_role(xObj) in [Atspi.Role.TREE, Atspi.Role.TREE_ITEM]:
                    return False
                elif Atspi.Accessible.get_role(obj) == Atspi.Role.HEADING and self.hasNoSize(obj):
                    return False
                elif Atspi.Accessible.get_role(xObj) == Atspi.Role.HEADING and self.hasNoSize(xObj):
                    return False
    ...
    def isFocusModeWidget(self, obj):
        ...
        if role == Atspi.Role.LIST_ITEM:
            rv = AXObject.find_ancestor(obj, lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.LIST_BOX)
            if rv:
    ...
    def treatAsDiv(self, obj, offset=None):
        ...
        if validRoles:
            if not childCount:
                ...
                rv = bool([x for x in obj if x and Atspi.Accessible.get_role(x) not in validRoles])

        if not rv:
            validRoles = self._validChildRoles.get(obj.parent)
            if validRoles:
                rv = bool([x for x in obj.parent if x and Atspi.Accessible.get_role(x) not in validRoles])
    ...
    def isComment(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) == Atspi.Role.COMMENT:
            return True
    ...
    def isContentDeletion(self, obj):
        ...
        # Remove this check when we bump dependencies to 2.34
        try:
            if Atspi.Accessible.get_role(obj) == Atspi.Role.CONTENT_DELETION:
                return True
        except:
            pass
    ...
    def isContentError(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) not in self._textBlockElementRoles():
            return False
    ...
    def isContentInsertion(self, obj):
        ...
        # Remove this check when we bump dependencies to 2.34
        try:
            if Atspi.Accessible.get_role(obj) == Atspi.Role.CONTENT_INSERTION:
                return True
        except:
            pass
    ...
    def isContentMarked(self, obj):
        ...
        # Remove this check when we bump dependencies to 2.36
        try:
            if Atspi.Accessible.get_role(obj) == Atspi.Role.MARK:
                return True
        except:
            pass
    ...
    def isContentSuggestion(self, obj):
        ...
        # Remove this check when we bump dependencies to 2.36
        try:
            if Atspi.Accessible.get_role(obj) == Atspi.Role.SUGGESTION:
                return True
        except:
            pass
    ...
    def isInlineIframe(self, obj):
        if not (obj and Atspi.Accessible.get_role(obj) == Atspi.Role.INTERNAL_FRAME):
            return False
    ...
    def isMathTopLevel(self, obj):
        return Atspi.Accessible.get_role(obj) == Atspi.Role.MATH
        ...
        def _include(x):
            ...
            if ((self.isTextBlockElement(obj) or self.isLink(obj)) and not displayedText) \
               ...
               or self.isLabellingContents(obj, contents):
                rv = False
            elif Atspi.Accessible.get_role(obj) == Atspi.Role.TABLE_ROW:
                ...
                if widget and (inferLabels or Atspi.Accessible.get_role(widget) in alwaysFilter):
                    rv = False
    ...
    def coordinatesForCell(self, obj, preferAttribute=True, findCellAncestor=False):
        roles = [Atspi.Role.TABLE_CELL,
                 ...
                 Atspi.Role.COLUMN_HEADER,
                 Atspi.Role.ROW_HEADER]
        if not (obj and Atspi.Accessible.get_role(obj) in roles):
            if not findCellAncestor:
                return -1, -1

            cell = AXObject.find_ancestor(obj, lambda x: x and Atspi.Accessible.get_role(x) in roles)
    ...
    def isEntryDescendant(self, obj):
        ...
        isEntry = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.ENTRY
    ...
    def isLabelDescendant(self, obj):
        ...
        isLabel = lambda x: x and Atspi.Accessible.get_role(x) in [Atspi.Role.LABEL, Atspi.Role.CAPTION]
    ...
    def isNavigableToolTipDescendant(self, obj):
        ...
        isToolTip = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.TOOL_TIP
        if isToolTip(obj):
            ancestor = obj
        else:
    ...
    def isToolBarDescendant(self, obj):
        ...
        isToolBar = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.TOOL_BAR
    ...
    def isWebAppDescendant(self, obj):
        ...
        isEmbedded = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.EMBEDDED
    ...
    def isDetachedDocument(self, obj):
        ...
        if (obj and Atspi.Accessible.get_role(obj) in docRoles):
    ...
    def iframeForDetachedDocument(self, obj, root=None):
        ...
        isIframe = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.INTERNAL_FRAME
    ...
    def _objectBoundsMightBeBogus(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) != Atspi.Role.LINK or not AXObject.supports_text(obj):
            return False
    ...
    def isLinkAncestorOfImageInContents(self, link, contents):
        ...
        for obj, start, end, string in contents:
            if Atspi.Accessible.get_role(obj) != Atspi.Role.IMAGE:
                continue
    ...
    def isLabellingContents(self, obj, contents=[]):
        ...
        for acc, start, end, string in contents:
            ...
            if ancestor and Atspi.Accessible.get_role(ancestor) in [Atspi.Role.LABEL, Atspi.Role.CAPTION]:
                return True
    ...
    def isEmptyToolTip(self, obj):
        return obj and Atspi.Accessible.get_role(obj) == Atspi.Role.TOOL_TIP \
            and self.queryNonEmptyText(obj) is None

    def isBrowserUIAlert(self, obj):
        if not (obj and Atspi.Accessible.get_role(obj) == Atspi.Role.ALERT):
            return False
    ...
    def isTopLevelBrowserUIAlert(self, obj):
        ...
        return Atspi.Accessible.get_role(parent) == Atspi.Role.FRAME
    ...
    def isClickableElement(self, obj):
        ...
        if rv and not obj.name and AXObject.supports_text(obj):
            ...
            if not string.strip():
                rv = Atspi.Accessible.get_role(obj) not in [Atspi.Role.STATIC, Atspi.Role.LINK]
    ...
    def isFakePlaceholderForEntry(self, obj):
        ...
        entry = AXObject.find_ancestor(obj, lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.ENTRY)
        if not (entry and entry.name):
            return False

        def _isMatch(x):
            try:
                role = Atspi.Accessible.get_role(x)
    ...
    def isListDescendant(self, obj):
        ...
        isList = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.LIST
    ...
    def isFeedArticle(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) != Atspi.Role.ARTICLE:
            return False
    ...
    def isLandmark(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) == Atspi.Role.LANDMARK:
            rv = True
    ...
    def isLink(self, obj):
        ...
        if role == Atspi.Role.LINK and not self.isAnchor(obj):
           ...
           and Atspi.Accessible.get_role(obj.parent) == Atspi.Role.LINK \
    ...
    def isNonNavigablePopup(self, obj):
        ...
        rv = Atspi.Accessible.get_role(obj) == Atspi.Role.TOOL_TIP \
            and not obj.getState().contains(Atspi.StateType.FOCUSABLE)
    ...
    def hasUselessCanvasDescendant(self, obj):
        ...
        isCanvas = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.CANVAS
    ...
    def isTextSubscriptOrSuperscript(self, obj):
        ...
        return Atspi.Accessible.get_role(obj) in [Atspi.Role.SUBSCRIPT, Atspi.Role.SUPERSCRIPT]
    ...
    def isCustomImage(self, obj):
        ...
        if self.isCustomElement(obj) and self.hasExplicitName(obj) \
           ...
           and not re.search(r'[^\s\ufffc]', obj.queryText().getText(0, -1)):
            for child in obj:
                if Atspi.Accessible.get_role(child) not in [Atspi.Role.IMAGE, Atspi.Role.CANVAS] \
    ...
    def isUselessImage(self, obj):
        ...
        rv = True
        if Atspi.Accessible.get_role(obj) not in [Atspi.Role.IMAGE, Atspi.Role.CANVAS] \
        ...
        if rv and obj.getState().contains(Atspi.StateType.FOCUSABLE):
            rv = False
        if rv and Atspi.Accessible.get_role(obj.parent) == Atspi.Role.LINK and not self.hasExplicitName(obj):
    ...
    def hasVisibleCaption(self, obj):
        ...
        pred = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.CAPTION and self.isShowingAndVisible(x)
    ...
    def isSpinnerEntry(self, obj):
        ...
        if Atspi.Role.SPIN_BUTTON in [Atspi.Accessible.get_role(obj), Atspi.Accessible.get_role(obj.parent)]:
            return True
    ...
    def eventIsAutocompleteNoise(self, event, documentFrame=None):
        ...
        isListBoxItem = lambda x: x and x.parent and Atspi.Accessible.get_role(x.parent) == Atspi.Role.LIST_BOX
        isMenuItem = lambda x: x and x.parent and Atspi.Accessible.get_role(x.parent) == Atspi.Role.MENU
        isComboBoxItem = lambda x: x and x.parent and Atspi.Accessible.get_role(x.parent) == Atspi.Role.COMBO_BOX
    ...
    def _eventIsBrowserUIAutocompleteTextNoise(self, event):
        ...
        if Atspi.Accessible.get_role(orca_state.locusOfFocus) in roles \
    ...
    def eventIsBrowserUIPageSwitch(self, event):
        ...
        if not Atspi.Accessible.get_role(event.source) in roles:
            return False
    ...
    def isContentEditableWithEmbeddedObjects(self, obj):
        ...
        hasTextBlockRole = lambda x: x and Atspi.Accessible.get_role(x) in self._textBlockElementRoles() \
    ...
    def handleEventFromContextReplicant(self, event, replicant):
        ...
        replicantRole = Atspi.Accessible.get_role(replicant)
    ...
    def findContextReplicant(self, documentFrame=None, matchRole=True, matchName=True):
        ...
        if obj and matchRole:
            if Atspi.Accessible.get_role(obj) != oldRole:
                obj = None
