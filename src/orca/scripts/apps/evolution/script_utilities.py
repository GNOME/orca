...
class Utilities(WebKitGtk.Utilities, gtk.Utilities):
    ...
    def isReceivedMessageContent(self, obj):
        if not (obj and Atspi.Accessible.get_role(obj) == Atspi.Role.SECTION):
            return False
    ...
    def isComposeAutocomplete(self, obj):
        ...
        return topLevel and Atspi.Accessible.get_role(topLevel) == Atspi.Role.WINDOW
    ...
    def realActiveDescendant(self, obj):
        ...
        if Atspi.Accessible.get_role(obj) == Atspi.Role.LAYERED_PANE:
            isTreeTable = lambda x: x and Atspi.Accessible.get_role(x) == Atspi.Role.TREE_TABLE
