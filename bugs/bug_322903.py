...
"""Bug reproducer for bug 322903.  This standalone module talks
directly with the AT-SPI Registry via its IDL interfaces.  No Orca
logic or code is stuck in the middle.

To run this module, merely type 'python bug_322903.py'.

To reproduce bug 322903, follow these steps:

1) Run this module in an xterm
2) Load bug_322903.html in firefox
3) Type in the text area

You will see that the caret stays at position 0.  This is because
the call to getTextAtOffset specifies a caret position of 0 (the
caret will move to the offset position specified in getTextAtOffset).

The caret position should not be changed because of a call to
getTextAtOffset.
"""
