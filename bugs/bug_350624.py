...
between windows.  This standalone module talks directly with the
AT-SPI Registry via its IDL interfaces.  No Orca logic or code is
stuck in the middle.

To run this module, merely type 'python bug_350625.py' in an xterm.

To reproduce bug 350625, follow these steps:

1) Run this module in an xterm.
2) Press Alt+Tab to cycle between windows.

This module should output messages such as the following (we care 
most about the object:property-change:accessible-name events):

vvvvvvvvvvvvvvv object:property-change:accessible-name vvvvvvvvvvvvvvv
EVENT:  object:property-change:accessible-name 0 0 <CORBA.any of type 'IDL:Accessibility/EventDetails:1.0'>
SOURCE: name='wwalker@wwalker-laptop: ~/gnome-head/orca/bugs' role='statusbar' state='ENABLED MULTI_LINE SENSITIVE SHOWING VISIBLE'

ANCESTRY:
+-name='metacity' role='application' state=''
  +-name=None role='window' state='ENABLED RESIZABLE SENSITIVE SHOWING VISIBLE'
    +-name=None role='panel' state='ENABLED SENSITIVE SHOWING VISIBLE'
      +-name=None role='filler' state='ENABLED SENSITIVE SHOWING VERTICAL VISIBLE'
        +-name='wwalker@wwalker-laptop: ~/gnome-head/orca/bugs' role='statusbar' state='ENABLED MULTI_LINE SENSITIVE SHOWING VISIBLE'
^^^^^^^^^^^^^^^ object:property-change:accessible-name ^^^^^^^^^^^^^^^

With metacity 2.15.13, we get nothing.
"""
...
def notifyEvent(event):
    ...
    if event.source.getRoleName() != "statusbar":
        return

    print
    print "vvvvvvvvvvvvvvv %s vvvvvvvvvvvvvvv" % event.type
    print "EVENT: ", event.type, event.detail1, event.detail2, event.any_data
    print "SOURCE: " + getAccessibleString(event.source)
    print
    print "ANCESTRY:"
