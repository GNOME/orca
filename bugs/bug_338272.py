...
To run this module, merely type 'python names_and_labels.py'.

To reproduce bug x, follow these steps:

1) Run this module in an xterm
2) Start Firefox, open the downloads window (Ctrl+Y) and make
   sure there are elements in the list box (you need to have
   done some downloads)
3) Arrow down to a list item and tab to the "Open" link/button.
4) In the output of this module, you will often see the following:

focus: 0 0 <CORBA.any of type 'IDL:Accessibility/EventDetails:1.0'>
  name=None role='push button' state='ENABLED FOCUSED SENSITIVE SHOWING VISIBLE' text=None
  LABELLED BY:None text=None

With this, Orca has no way of knowing what the name of the object is and can only present the fact that the user is on a push button.  The desirable behavior is to give Orca an object with a name, and better yet, one that implements the accessible text interface so Orca can know more about exactly what is being painted on the display.
"""
