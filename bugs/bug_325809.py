...
"""Bug reproducer for bug 325809.  This standalone module talks
directly with the AT-SPI Registry via its IDL interfaces.  No Orca
logic or code is stuck in the middle.

To run this module, merely type 'python bug_325809.py'.

To reproduce bug 325809, follow these steps:
...
3) Open the 'Editable Cells' or 'List Store' or 'Tree Store' demos under
   'Tree View'
4) Make sure the table in the chosen demo has focus
5) Position the mouse pointer over a cell and press F11, watching the output
   in the xterm where you are running the standalone application

You will find that the accessible returned is actually the cell
underneath the cell with the pointer.  This is even the case when the
pointer is over a column header.  It's as though the column header
doesn't exist.
"""
...
def notifyEvent(event):
    ...
    global currentY
    global focusedObj
    if event.type == "mouse:abs":
        currentX = event.detail1
        currentY = event.detail2
    if event.type == "focus:":
        focusedObj = event.source
        print "Focus is " + getAccessibleString(event.source)
        
def notifyKeystroke(event):
    if event.event_string == "F12":
        shutdownAndExit()
    elif event.event_string == "F11":
        if focusedObj and (event.type == Accessibility.KEY_PRESSED_EVENT):
            component = focusedObj.queryInterface(\
                "IDL:Accessibility/Component:1.0")
            if component:
                component = component._narrow(Accessibility.Component)
                acc = component.getAccessibleAtPoint(currentX, currentY, 0)
                if acc:
                    print "Cell at (%d, %d): %s" \
                          % (currentX, currentY, getAccessibleString(acc))
                else:
                    print "Cell at (%d, %d): None" % (currentX, currentY)
        return True
    return False
...
def test():
    ...
    print
    print "1) Put focus on a table."
    print "2) Position mouse over a cell."
    print "3) Press F11 to see what getAccessibleAtPoint reveals."
    print
