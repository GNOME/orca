...
For information on how to execute these commands, see [README-REMOTE-CONTROLLER.md](README-REMOTE-CONTROLLER.md).
...
#### Commands (use with `ExecuteCommand`)
...
#### Settings (use with `ExecuteRuntimeGetter` / `ExecuteRuntimeSetter`)
...
- **`ContractionTable`**: Gets/Sets the current braille contraction table name.
...
- **`VerbosityLevel`**: Gets/Sets the current braille verbosity level for object presentation.
...
#### Parameterized Commands (use with `ExecuteParameterizedCommand`)
...
- **`CapitalizationStyle`**: Gets/Sets the current capitalization style.
- **`CurrentServer`**: Gets/Sets the name of the current speech server (Speech Dispatcher or Spiel).
- **`CurrentSynthesizer`**: Gets/Sets the current synthesizer of the speech server.
- **`CurrentVoice`**: Gets/Sets the current voice name.
...
- **`Pitch`**: Gets/Sets the current speech pitch.
- **`PunctuationLevel`**: Gets/Sets the current punctuation level.
- **`Rate`**: Gets/Sets the current speech rate.
...
- **`SpeechIsEnabled`**: Gets/Sets whether the speech server is enabled. See also the is-muted getter.
...
- **`VerbosityLevel`**: Gets/Sets the current speech verbosity level for object presentation.
- **`Volume`**: Gets/Sets the current speech volume.
...
- **`DateFormat`**: Gets/Sets the current date format name.
- **`TimeFormat`**: Gets/Sets the current time format name.
...
- **`KeyEchoEnabled`**: Gets/Sets whether echo of key presses is enabled. See also get_character_echo_enabled.
